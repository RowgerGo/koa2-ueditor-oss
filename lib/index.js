...
const ueditor = function(dir, options) {
    const ossOption={
        endpoint: dir[1].oss.endpoint,
        accessKeyId: dir[1].oss.accessKeyId,
        accessKeySecret: dir[1].oss.accessKeySecret,
        bucket: dir[1].oss.bucket,
    }

    let client = new OSS(ossOption);
    let ueOpts = []
    ...
    return async (ctx, next) => {
        ...
        if(Object.keys(uploadType).includes(action)){
            ...
            try {
                ...
                if(action === conf.scrawlActionName){
                    ...
                    console.log("====================path.joinpublicDir, ...pathFormat",path.join(publicDir, ...pathFormat))
                ...
                // 表单上传图片、文件
                    ...
                    const fileUrl=path.join(publicDir,result.url)
                    let ossRes = await client.put(result.url, fileUrl);
                    const ossUrl=ossRes.url
        ...
        // 返回Ueditor配置给前端
            ...
            conf.imageUrlPrefix=dir[1].oss.region;
            result = conf
        }
        else{
            result = {state: 'FAIL'}
